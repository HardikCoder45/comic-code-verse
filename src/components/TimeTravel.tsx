  ...
  return <div className="container mx-auto py-8 px-4">
      <div className="mb-8 text-center">
        ...
        <SpeechBubble type="speech" color="blue" position="top" className="inline-block">
          <p className="font-comic text-black">Travel through my career timeline! Use keyboard arrows ← → or buttons to navigate</p>
          <p className="font-comic text-sm mt-1 text-black">Press <kbd className="px-2 py-1 bg-gray-200 rounded">ESC</kbd> to return from detailed view</p>
        </SpeechBubble>
      </div>

      {/* Keyboard indicator */}
      {isKeyboardActive && <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50">
      ...
      <div className="relative h-28 mb-12 bg-gradient-to-r from-purple-100 via-blue-100 to-pink-100 rounded-xl border-2 border-comic-border overflow-hidden shadow-lg">
        <div className="absolute top-0 left-0 w-full h-full flex items-center">
          <div className="relative w-full h-2 bg-gray-300">
            {yearsAvailable.map((year, index) => <button key={year} className={`absolute transform -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full 
                  ${year === currentYear ? 'bg-comic-green border-2 border-black z-10 scale-125 shadow-lg' : 'bg-gray-400 hover:bg-gray-500'} 
        ...
        <div className="absolute bottom-4 left-0 w-full px-4">
          <div className="relative w-full h-8">
            {yearsAvailable.map((year, index) => <div key={`label-${year}`} className={`absolute transform -translate-x-1/2 font-bangers text-lg
                  ${year === currentYear ? 'text-comic-blue scale-125' : 'text-gray-500'}`} style={{
            left: calculatePosition(index)
          }}>
                {year}
              </div>)}
      ...
      <div className="flex justify-center mb-8 space-x-4">
        <button onClick={handlePrevYear} disabled={currentYearIndex === 0} className="comic-button-secondary flex items-center disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Previous year">
          <ChevronLeft size={24} />
        ...
        <button onClick={handleNextYear} disabled={currentYearIndex === yearsAvailable.length - 1} className="comic-button-secondary flex items-center disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Next year">
          <span className="mr-1">Next</span>
          <ChevronRight size={24} />
        </button>
      </div>

      {viewMode === 'list' ? (/* Year events list view */
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {currentEvents.map((event, index) => <motion.div key={`${event.year}-${event.title}`} className="animate-pop cursor-pointer" whileHover={{
        scale: 1.02
      }} whileTap={{
        scale: 0.98
      }} style={{
        animationDelay: `${index * 0.1}s`
              ...
              <ComicPanel title={event.title} headerColor={event.color} className="h-full">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    {event.techStack && <div className="flex flex-wrap gap-1 mt-2">
                        {event.techStack.slice(0, 3).map((tech, i) => <span key={i} className="text-xs bg-gray-100 px-2 py-1 rounded-full text-gray-700">
                            {tech}
                          </span>)}
                        {event.techStack.length > 3 && <span className="text-xs bg-gray-100 px-2 py-1 rounded-full text-gray-700">
    ...
    selectedEvent && <motion.div initial={{
      opacity: 0,
      y: 20
    }} animate={{
      opacity: 1,
      ...
      y: -20
          ...
          setViewMode('list');
            ...
            {selectedEvent.subEvents && <div className="my-6">
                ...
                <div className="relative pl-8 border-l-2 border-dashed border-comic-purple space-y-4">
                  {selectedEvent.subEvents.map((subEvent, index) => <motion.div key={index} className="relative" initial={{
            opacity: 0,
            x: -10
          }} animate={{
            opacity: 1,
            ...
            {/* Tech Stack */}
            {selectedEvent.techStack && <div className="mb-6">
                <h3 className="text-xl font-bangers text-comic-green mb-2">Tech Stack</h3>
                <div className="flex flex-wrap gap-2">
                  {selectedEvent.techStack.map((tech, i) => <span key={i} className="px-3 py-1 bg-gray-100 rounded-full text-gray-700 text-sm">
                      {tech}
                    </span>)}
                </div>
              </div>}
            ...
            {/* Testimonial */}
            {selectedEvent.testimonial && <div className="mb-6">
            ...
            {/* Impact */}
            {selectedEvent.impact && <div className="mb-6">
      ...
      <div className="mt-12 p-4 bg-gray-100 rounded-lg text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {yearsAvailable.map(year => <button key={`quick-${year}`} onClick={() => {
          setCurrentYear(year);
          setSelectedEvent(null);
          setViewMode('list');
        }} className={`px-3 py-1 rounded-md text-sm 
                ${year === currentYear ? 'bg-comic-blue text-white font-bold' : 'bg-gray-200 hover:bg-gray-300'}`}>
              {year}
            </button>)}
        </div>
      </div>
