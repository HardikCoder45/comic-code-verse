...
const ChatBot = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([{
    role: 'assistant',
    content: "Hey there! I'm Hardik, your friendly AI assistant. How can I help you today? 😊"
  }]);
  ...
  return <div className="fixed bottom-24 right-6 z-[60]">
      {/* Chat toggle button */}
      <button onClick={toggleChat} className="bg-comic-orange text-white rounded-full w-14 h-14 flex items-center justify-center border-2 border-black shadow-md hover:bg-comic-pink transition-colors" aria-label={isOpen ? "Close chat" : "Open chat"}>
        ...
        scale: 0.9
            ...
            <div className="bg-comic-blue text-white p-3 flex justify-between items-center">
              ...
              <div className="flex items-center space-x-2">
                <button onClick={toggleMinimize} className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition">
                  {isMinimized ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                </button>
                <button onClick={toggleChat} className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition">
                  <X size={14} />
                </button>
              </div>
            </div>
            ...
            duration: 0.2
                  ...
                  <div className="p-3 h-80 overflow-y-auto bg-gray-50">
                    {messages.map((msg, index) => <div key={index} className={`mb-3 ${msg.role === 'user' ? 'text-right' : ''}`}>
                  ...
                  <form onSubmit={handleSubmit} className="p-3 border-t-2 border-comic-border bg-white">
                    <div className="flex items-center">
                      <input type="text" value={message} onChange={e => setMessage(e.target.value)} placeholder="Type a message..." className="flex-grow p-2 border-2 border-comic-border rounded-lg focus:outline-none focus:ring-2 focus:ring-comic-blue font-comic" disabled={isLoading} />
                      <button type="submit" disabled={isLoading || !message.trim()} className="ml-2 w-10 h-10 bg-comic-green text-white rounded-full flex items-center justify-center border-2 border-black disabled:opacity-50">
                        <Send size={18} />
                      </button>
                    </div>
                  </form>
