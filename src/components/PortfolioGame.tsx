...
const gameLevels: GameLevel[] = [
  ...
  // Level 2: Intermediate
  {
    ...
    obstacles: [
      ...
      { x: 350, y: 300, width: 20, height: 200 },
...
const PortfolioGame: React.FC = () => {
  ...
  const [powerups, setPowerups] = useState<{
    speedBoost: boolean;
    bugShield: boolean;
  }>({ speedBoost: false, bugShield: false });
  ...
  useEffect(() => {
    ...
    if (powerups.bugShield) {
      const timer = setTimeout(() => {
        setPowerups(prev => ({ ...prev, bugShield: false }));
  ...
  const startNextLevel = () => {
    if (currentLevel < gameLevels.length - 1) {
      ...
      setPowerups({ speedBoost: false, bugShield: false });
      setCollectedItems([]);
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="text-center mb-8">
        ...
        <SpeechBubble type="speech" color="pink" position="top" className="inline-block">
          <p className="font-comic">Master programming skills and build projects in this challenging game!</p>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="text-white">
            ...
            {Array.from({length: lives}).map((_, i) => (
              <Heart key={i} size={16} className="inline text-red-300 ml-1" fill="white" />
            ))}
          </div>
        </div>
      ...
      {/* Power-ups status */}
      {gameStarted && !gameOver && (powerups.speedBoost || powerups.bugShield) && (
        <div className="mb-4 p-2 bg-comic-green text-white rounded-lg flex justify-center space-x-4">
          {powerups.speedBoost && (
            <div className="flex items-center">
      ...
      <div 
        ref={gameAreaRef} 
        className="relative w-full h-[500px] rounded-xl border-4 border-comic-border overflow-hidden"
        style={{ 
      ...
      >
        {!gameStarted ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white bg-opacity-90 z-20">
            ...
            <p className="font-comic text-center max-w-md mb-2">
              {lives > 0 
                ? "Congratulations! You've mastered the portfolio game." 
                : "Game Over! You ran out of lives."}
            </p>
            <p className="font-comic text-2xl mb-6">Final Score: {totalScore + (levelCompleted ? score : 0)}</p>
            <div className="grid grid-cols-3 gap-4 mb-6">
              {collectedItems
                .filter(item => item.type !== 'obstacle' && item.type !== 'powerup')
                .map(item => (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white bg-opacity-90 z-20">
            ...
            {currentLevel < gameLevels.length - 1 ? (
              <button onClick={startNextLevel} className="comic-button">
        ...
        {/* Game obstacles */}
        {gameStarted && !gameOver && !levelCompleted && gameLevels[currentLevel].obstacles.map((obstacle, index) => (
          <div
            key={`obstacle-${index}`}
        ...
        {/* Game objects */}
        {gameStarted && !gameOver && !levelCompleted && objects.map(obj => !obj.collected && (
          <motion.div
            key={obj.id}
            className="absolute"
            style={{
        ...
        {gameStarted && !gameOver && !levelCompleted && (
          <motion.div
            className={`absolute rounded-full border-2 border-black flex items-center justify-center ${
              powerups.bugShield ? "bg-comic-green ring-4 ring-comic-yellow" : "bg-comic-green"
            } ${powerups.speedBoost ? "animate-pulse" : ""}`}
            style={{
              left: position.x,
              top: position.y,
        ...
        {showInfo && (
          ...
          >
            <SpeechBubble 
              type="speech" 
              color={
                showInfo.type === 'obstacle' ? 'orange' : 
                showInfo.type === 'powerup' ? 'yellow' : 'green'
              } 
              position="bottom"
            >
