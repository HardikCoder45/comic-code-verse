...
import { ArrowRight, Code, Github, Zap, Star, Users, BookOpen, Award, Rocket, Coffee, Brain, Database, PanelLeft, PanelRight, MessageSquare, Heart, ExternalLink, Sparkles, Gamepad, Volume2, VolumeX } from 'lucide-react';
...
const Landing = () => {
  ...
  const backgroundImages = [
    'https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?auto=format&fit=crop&w=1600',
    'https://images.unsplash.com/photo-1426604966848-d7adac402bff?auto=format&fit=crop&w=1600',
    'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1600',
  ];
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (bookRef.current) {
        const rect = (bookRef.current as HTMLElement).getBoundingClientRect();
  ...
  // Automatically cycle through panels
  useEffect(() => {
    const interval = setInterval(() => {
      setActivePanel(prev => (prev + 1) % backgroundImages.length);
    }, 7000);
  ...
  return (
    <div className="min-h-screen w-full perspective-1000 bg-gradient-to-b from-amber-50 to-orange-50 dark:from-gray-900 dark:to-indigo-950" ref={containerRef}>
      {/* Sound and theme controls */}
      <div className="fixed top-4 right-4 z-50 flex space-x-3">
        <motion.button
          className="p-2 rounded-full bg-white/20 backdrop-blur-md shadow-lg hover:bg-white/30 transition-colors"
        ...
        >
          {isMuted ? (
            <VolumeX size={18} className="text-gray-800 dark:text-white" />
          ) : (
            <Volume2 size={18} className="text-gray-800 dark:text-white" />
          )}
        </motion.button>
        ...
        <div className="absolute inset-0 ml-12 border-8 border-comic-blue bg-comic-background rounded-r-md overflow-hidden">
          ...
          <div className={`relative h-full flex items-center justify-center overflow-hidden transition-all duration-700 ${showElements.hero ? 'opacity-100' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="absolute inset-0 z-0 overflow-hidden">
              ...
              <motion.div 
                className="absolute inset-0 bg-gradient-to-b from-transparent via-blue-900/30 to-comic-blue/40 mix-blend-overlay"
              />
            </div>
              ...
              >
                {['Home', 'Projects', 'About', 'Skills', 'Contact'].map((item, index) => (
                  <Link 
                    key={item} 
                    to={`/${item.toLowerCase()}`} 
                    className="font-comic font-bold hover:text-comic-blue transition-colors relative overflow-hidden group text-white"
                    onMouseEnter={() => playSound('hover')}
                  >
                    <motion.span
                      whileHover={{ y: -3 }}
            ...
            <div className="relative max-w-6xl mx-auto text-center px-4 z-10">
              <div className="relative">
                <motion.h1 
                  className="font-bangers text-5xl md:text-7xl mb-6 leading-tight relative z-10 text-white" 
                  style={{
                ...
                >
                  <motion.span 
                    className="text-comic-blue block bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300 text-transparent" 
                  ...
                  <motion.span 
                    className="text-comic-pink block -mt-2 bg-clip-text bg-gradient-to-r from-pink-400 to-rose-300 text-transparent" 
                    initial={{ y: 50, opacity: 0 }} 
                    animate={{ y: 0, opacity: 1 }} 
                  ...
                  <motion.span 
                    className="text-comic-orange block -mt-2 bg-clip-text bg-gradient-to-r from-orange-400 to-amber-300 text-transparent" 
                    initial={{ y: 50, opacity: 0 }} 
                    animate={{ y: 0, opacity: 1 }} 
              ...
              >
                {[
                  { icon: <Gamepad size={16} />, text: "Interactive Games" },
                  { icon: <Award size={16} />, text: "Achievement System" },
                  { icon: <BookOpen size={16} />, text: "Comic Style" },
                  { icon: <Star size={16} />, text: "Portfolio Showcase" }
                ].map((item, i) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    ...
                    <span className="text-white text-sm font-comic">{item.text}</span>
                  </motion.div>
                ))}
              </motion.div>
            </div>
      ...
      >
        ...
        >
          ...
          >
            <ArrowRight size={16} className="text-black/70 dark:text-white/70" />
          </motion.span>
          <motion.p 
            className="font-comic text-black/70 dark:text-white/70 text-sm"
          >
          ...
          <motion.span
            ...
            transition={{ duration: 5, repeat: Infinity }}
          >
            <ArrowRight size={16} className="text-black/70 dark:text-white/70 transform rotate-180" />
          </motion.span>
        </motion.div>
      </motion.div>
